...
<template>
	<div class="top">
		...
		<div class="nextContainer" v-if="currentPointInfo">
			...
			<el-button @click='goNext()' :disabled="currentPointInfo.playIndex === store.poetInfo.length-1"
				class="next_btn">
				<template #icon>
					<img src="../static/next.png" alt="">
				</template>
		...
		<div class="palyContainer" v-if="currentPointInfo">
			<el-button class="play" :icon="VideoPlay" @click='play()' v-if="isPlay" style="color: lightgreen;"
				:disabled="currentPointInfo.playIndex === store.poetInfo.length-1" round />
